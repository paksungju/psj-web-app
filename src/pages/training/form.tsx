...
function makeEmptyForm(): ApiAppPayload {
  return {
    app_id: 8,
    gr_num: 0,
    reply_cd: 0,
...
export default function TrainingFormPage() {
  ...
  const menuCd = 'training'
  const savePath = 'training'
  const basePath = '/training'

  const [form, setForm] = useState<ApiAppPayload>(() => ({ ...makeEmptyForm() }))
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setAttachmentsBySlot((prev) => Array.from({ length: count }, (_, i) => prev[i] ?? []))
      } catch (e) {
        console.error(e)
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (savedId) navigate(`/training/${savedId}/form`)
    ...
    <Box sx={{ flexGrow: 1, overflow: 'auto', p: 3 }}>
      ...
      >
        <Typography variant="h5" sx={{ mb: 2, fontWeight: 600 }}>
          {isEdit ? '트레이닝 데이터 수정' : '트레이닝 데이터 등록'}
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          {isEdit ? '트레이닝 데이터를 수정합니다.' : '새 트레이닝 데이터를 등록합니다.'}
        </Typography>

        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Stack spacing={2.5}>
            <Box sx={{ display: 'none' }}>
              <TextField type="number" value={form.app_id ?? 8} onChange={handleChange('app_id')} />
              <TextField type="number" value={form.data_id ?? 0} onChange={handleChange('data_id')} />
            </Box>
            ...
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 0.5 }}>내용</Typography>
            ...
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>이미지</Typography>
              <input
                ref={imageInputRef}
                type="file"
                accept="image/*"
              ...
              <Stack direction="row" spacing={1.5} sx={{ mt: 1.5 }}>
                <Button size="small" variant="outlined" onClick={() => imageInputRef.current?.click()} disabled={uploadingImages}>
                  +이미지등록
                </Button>
                <Button size="small" variant="outlined" onClick={handleInsertToEditor} disabled={attachedImages.length === 0 || selectedImageId == null}>
                  에디터삽입
                </Button>
              </Stack>
              <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
                ...
                <Box sx={{ flex: 1, border: '1px solid', borderColor: 'divider', borderRadius: 1, maxHeight: 200, overflow: 'auto' }}>
                  {attachedImages.length === 0 ? (
                    ...
                    <Box component="ul" sx={{ m: 0, p: 0, listStyle: 'none' }}>
                      {attachedImages.map((item) => {
                        const keyId = item.type === 'local' ? item.id : item.fileId
                        ...
                        const selected = selectedImageId === keyId
                        return (
                          <Box
                            component="li"
                            key={keyId}
                            onClick={() => setSelectedImageId(keyId)}
                            sx={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: 1,
            ...
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>첨부파일</Typography>
              <input
              ...
              {Array.from({ length: uploadCount }, (_, slotIndex) => (
                <Box key={slotIndex} sx={{ mb: 2 }}>
                  <Stack direction="row" spacing={1.5} alignItems="stretch" useFlexGap>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0, border: '1px solid', borderColor: 'divider', borderRadius: 1, minHeight: 40, maxHeight: 40, overflow: 'auto' }}>
          ...
          <Stack direction="row" spacing={1.5} sx={{ mt: 3, justifyContent: 'flex-end' }}>
            <Button variant="outlined" color="inherit" onClick={() => navigate(basePath)} disabled={saving}>
              취소
            </Button>
