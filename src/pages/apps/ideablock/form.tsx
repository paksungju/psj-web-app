...
import { ContentListSidebar } from './view'

const APP_ID = 10
const BASE_PATH = '/apps/ideablock'
const MENU_CD = 'ideablock'
const SAVE_PATH = 'ideablock'
...
function isRootPost(rc: ApiAppPayload['reply_cd']): boolean {
...
export default function IdeaBlockFormPage() {
  ...
  const [form, setForm] = useState<ApiAppPayload>(() => ({ ...makeEmptyForm(APP_ID) }))
  ...
  const sidebarSelectedContentId = isReply
    ? parentId ?? null
    : isEdit
      ? isRootPost(form.reply_cd)
        ? form.data_id ?? null
        : form.parent_id ?? form.gr_num ?? null
      : null
  const sidebarCurrentDataId = isEdit ? form.data_id ?? null : null
    ...
    if (isReply && id && !isNaN(parseInt(id, 10))) {
      ...
      const loadParent = async () => {
        try {
          ...
          console.error(e)
          if (!cancelled) navigate(BASE_PATH)
        } finally {
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      try {
        const res = await fetchImagesByDataApi({ menuCd: MENU_CD, dataId })
        if (cancelled) return
  ...
  useEffect(() => {
    ...
    const loadAttachments = async () => {
      try {
        const res = await fetchFilesByDataApi({ menuCd: MENU_CD, dataId, limit: 200 })
        if (cancelled) return
      ...
      <Box sx={INFO_BOARD_PAGE_SX}>
        ...
        <Paper elevation={0} sx={INFO_BOARD_PAPER_SX}>
          <Box sx={INFO_BOARD_LAYOUT_SX}>
            <ContentListSidebar
              selectedContentId={sidebarSelectedContentId}
              currentDataId={sidebarCurrentDataId}
            />
                    ...
                    {attachedImages.length === 0 ? (
                      ...
                      <Box component="ul" sx={{ m: 0, p: 0, listStyle: 'none' }}>
                        {attachedImages.map((item) => {
                          const iid = item.type === 'local' ? item.id : item.fileId
                          ...
                          const selected = selectedImageId === iid
                          return (
                            <Box
                              component="li"
                              key={iid}
                              onClick={() => setSelectedImageId(iid)}
                              sx={{
                                display: 'flex',
                                alignItems: 'center',
                                gap: 1,
